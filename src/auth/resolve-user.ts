	...
	return await db.transaction(async (tx) => {
		...
		if (
			...
			adminEmails
				.split(",")
				.map((e) => e.trim())
				.includes(emailFromToken)
		) {
			let [org] = await tx
				.select()
				.from(schema.orgs)
				.where(eq(schema.orgs.code, "system"))
				.limit(1);
			if (!org)
				[org] = await tx
					.insert(schema.orgs)
					.values({ code: "system", name: "System Organization" })
					.returning();

			let [app] = await tx
				.select()
				.from(schema.apps)
				.where(eq(schema.apps.code, "ironclad-admin"))
				.limit(1);
			if (!app)
				[app] = await tx
					.insert(schema.apps)
					.values({ code: "ironclad-admin", name: "Ironclad Admin" })
					.returning();
			...
			let [appOrg] = await tx
				...
				.where(
					and(
						eq(schema.appsOrgs.appId, app.id),
						eq(schema.appsOrgs.orgId, org.id),
					),
				)
				.limit(1);
			if (!appOrg)
				[appOrg] = await tx
					...
					.values({ appId: app.id, orgId: org.id })
					.returning();
			...
			let [role] = await tx
				...
				.from(schema.roles)
				.where(
					and(eq(schema.roles.appId, app.id), eq(schema.roles.code, "admin")),
				)
				.limit(1);
			if (!role)
				[role] = await tx
			...
			let [priv] = await tx
				...
				.where(
					and(
						eq(schema.privileges.appId, app.id),
						eq(schema.privileges.code, "manage:platform"),
					),
				)
				.limit(1);
			if (!priv)
			...
			await tx
				.insert(schema.rolePrivileges)
				.values({ roleId: role.id, privilegeId: priv.id })
				.onConflictDoNothing();
			...
			const existingMem = await tx
				...
				.where(
					and(
						...
						eq(schema.memberships.appOrgId, appOrg.id),
						eq(schema.memberships.roleId, role.id),
					),
				)
				.limit(1);
			if (!existingMem[0]) {
				await tx
					...
					.values({ userId: user.id, appOrgId: appOrg.id, roleId: role.id });
			}
		}
