...
adminRouter.use("*", async (c, next) => {
	const { evaluateAuthorization } = await import(
		"./authorization/evaluate-authorization.js"
	);
	...
	if (!allowed) {
		return c.json(
			{ error: "Forbidden: Requires manage:platform privilege" },
			403,
		);
	}
	await next();
...
adminRouter.get("/apps", async (c) =>
	c.json(await c.get("db").select().from(apps)),
);
...
adminRouter.get("/orgs", async (c) =>
	c.json(await c.get("db").select().from(orgs)),
);
adminRouter.post("/orgs", async (c) => {
	const body = await c.req.json();
	const [newOrg] = await c
		.get("db")
		.insert(orgs)
		.values({
			code: body.code,
...
adminRouter.put("/orgs/:id", async (c) => {
	...
	const [updatedOrg] = await c
		...
		.update(orgs)
		.set(body)
		.where(eq(orgs.id, c.req.param("id")))
		.returning();
	return c.json(updatedOrg);
});

// AppOrgs
adminRouter.get("/app-orgs", async (c) =>
	c.json(await c.get("db").select().from(appsOrgs)),
);
...
adminRouter.delete("/app-orgs/:id", async (c) => {
	await c
		.get("db")
		.delete(appsOrgs)
		.where(eq(appsOrgs.id, c.req.param("id")));
	return c.json({ success: true });
});

// Roles
adminRouter.get("/roles", async (c) =>
	c.json(await c.get("db").select().from(roles)),
);
...
// Privileges
adminRouter.get("/privileges", async (c) =>
	c.json(await c.get("db").select().from(privileges)),
);
adminRouter.post("/privileges", async (c) => {
	const body = await c.req.json();
	const [newPriv] = await c
		.get("db")
		.insert(privileges)
		.values(body)
		.returning();
...
adminRouter.delete("/privileges/:id", async (c) => {
	await c
		.get("db")
		.delete(privileges)
		.where(eq(privileges.id, c.req.param("id")));
	return c.json({ success: true });
});

// Memberships
adminRouter.get("/memberships", async (c) =>
	c.json(await c.get("db").select().from(memberships).limit(100)),
);
adminRouter.post("/memberships", async (c) => {
	const body = await c.req.json();
	const [newMem] = await c
		.get("db")
		.insert(memberships)
		.values({
			userId: body.userId,
			appOrgId: body.appOrgId,
			roleId: body.roleId,
		})
		.returning();
...
adminRouter.put("/memberships/:id", async (c) => {
	const body = await c.req.json();
	const [updatedMem] = await c
		.get("db")
		.update(memberships)
		.set(body)
		.where(eq(memberships.id, c.req.param("id")))
		.returning();
...
adminRouter.delete("/role-privileges/:roleId/:privilegeId", async (c) => {
	...
	await c
		...
		.where(
			and(
				eq(rolePrivileges.roleId, c.req.param("roleId")),
				eq(rolePrivileges.privilegeId, c.req.param("privilegeId")),
			),
		);
	return c.json({ success: true });
});
